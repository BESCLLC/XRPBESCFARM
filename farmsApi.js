...
const processStakeTxEntry = async (entry, metaData, lpTokenCurrency, lpTokenIssuer, escrowAddress) => {
  ...
  const fallbackDelta = !delta && !amountMatches ? getAnyIouBalanceDelta(metaData, escrowAddress) : null
  const stakeValue =
    delta ||
...
const mountFarmsApi = (app) => {
  ...
  app.post(`${API_PREFIX}/farms/stake`, async (req, res) => {
    try {
      ...
      const fallbackDelta = !stakeDelta ? getAnyIouBalanceDelta(verified?.meta, escrowAddress) : null
      const stakeAmount =
