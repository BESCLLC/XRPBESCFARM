...
const verifyAction = async ({ account, action, farmId, txHash, destinations, expectAmount }) => {
  ...
  if (expectAmount?.type === 'iou') {
    if (!amount || amount.type !== 'iou') {
      throw new Error('Invalid payment asset')
    }
    if (expectAmount.currency && amount.currency !== expectAmount.currency) {
      throw new Error('Invalid LP token currency')
    }
    if (expectAmount.issuer && amount.issuer !== expectAmount.issuer) {
      throw new Error('Invalid LP token issuer')
    }
  }
