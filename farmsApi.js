...
const { Client, dropsToXrp, Wallet, getBalanceChanges } = require('xrpl')
...
const getIouBalanceDelta = (meta, account, currency, issuer) => {
  ...
  try {
    ...
    const entry = accountChanges.find(
      (change) => change.currency === currency && change.issuer === issuer,
    )
...
const processStakeTxEntry = async (entry, metaData, lpTokenCurrency, lpTokenIssuer, escrowAddress) => {
  ...
  const destination = txFields.Destination || txFields.destination
  if (destination !== escrowAddress) return false
  ...
  const stakeValue =
    ...
    (amount &&
      amount.type === 'iou' &&
      amount.currency === lpTokenCurrency &&
      amount.issuer === lpTokenIssuer &&
...
const indexEscrowStakes = async () => {
  ...
  try {
    ...
    if (!meta?.lpTokenCurrency || !meta?.lpTokenIssuer) return
    ...
    for (const entry of transactions) {
      ...
      await processStakeTxEntry(tx, metaData, meta.lpTokenCurrency, meta.lpTokenIssuer, escrowAddress)
    }
  } catch (error) {
...
const verifyAction = async ({ account, action, farmId, txHash, destinations, expectAmount }) => {
  ...
  const sender = txFields.Account || txFields.account
  if (sender !== account) {
    throw new Error('Account mismatch')
  }
  const destination = txFields.Destination || txFields.destination
  const allowedDestinations = (destinations || [FARM_SIGNAL_ADDRESS, BESC_ISSUER]).filter(Boolean)
  ...
  if (expectAmount?.type === 'iou') {
    const amountMatches =
      amount &&
      amount.type === 'iou' &&
      (!expectAmount.currency || amount.currency === expectAmount.currency) &&
      (!expectAmount.issuer || amount.issuer === expectAmount.issuer)
    if (!amountMatches) {
      const delta = getIouBalanceDelta(meta, destination, expectAmount.currency, expectAmount.issuer)
      if (!delta) {
        throw new Error('Invalid payment asset')
      }
      amount = {
