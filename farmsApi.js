...
const parseMemo = (memos = []) => {
  for (const memo of memos) {
    ...
    try {
      ...
      return { ...parsed, type: type || parsed.type }
    } catch (error) {
      // ignore invalid memo
    }
  }
...
const loadFarmMeta = async () => {
  ...
  const reserveXrp = amm?.amount ? Number(dropsToXrp(amm.amount)) : 0
  const reserveBesc = amm?.amount2?.value ? Number(amm.amount2.value) : 0
  const priceXrp =
...
const verifyAction = async ({ account, action, farmId, txHash }) => {
  ...
  if (tx.Destination !== FARM_SIGNAL_ADDRESS) {
    throw new Error('Invalid destination')
  }
  const memo = parseMemo(tx.Memos)
  if (!memo || memo.action !== action || memo.farmId !== farmId) {
