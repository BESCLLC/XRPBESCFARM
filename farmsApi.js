...
const FARMS_LP_CURRENCY = process.env.FARMS_LP_CURRENCY || ''
const FARMS_LP_ISSUER = process.env.FARMS_LP_ISSUER || ''
...
const loadFarmMeta = async () => {
  ...
  const [ammResult, statsResult, xrpUsdResult] = await Promise.allSettled([
    fetchAmmData(),
    fetchBescStats(),
    fetchXrpUsd(),
  ])
  ...
  const xrpUsd = xrpUsdResult.status === 'fulfilled' ? xrpUsdResult.value : null

  if (!amm && farmMetaCache.data) {
    logWarn('AMM data unavailable, reusing cached farm metadata')
    farmMetaCache = { data: farmMetaCache.data, fetchedAt: now() }
    return farmMetaCache.data
  }
  ...
  const lpTokenCurrency = FARMS_LP_CURRENCY || lpToken?.currency || null
  const lpTokenIssuer = FARMS_LP_ISSUER || lpToken?.issuer || null
...
const buildLpTrustlineTx = (account, currency, issuer) => ({
...
const getAccountBalanceChanges = (meta, account) => {
  if (!meta || !account) return []
  const accountKey = normalizeAddress(account)
  const changes = getBalanceChanges(meta)
  if (!changes) return []
  if (Array.isArray(changes)) {
    const entry = changes.find((item) => normalizeAddress(item?.account) === accountKey)
    return Array.isArray(entry?.balances) ? entry.balances : []
  }
  const accountChanges = changes?.[accountKey]
  return Array.isArray(accountChanges) ? accountChanges : []
}

const getPositiveIouDeltas = (meta, account) => {
  if (!meta || !account) return []
  try {
    const accountChanges = getAccountBalanceChanges(meta, account)
    if (!Array.isArray(accountChanges)) return []
    return accountChanges
      .filter((change) => change && change.currency && change.issuer)
      .map((change) => {
        const value = new BigNumber(change.value)
        if (!value.isFinite() || value.lte(0)) return null
        return {
          currency: normalizeCurrency(change.currency),
          issuer: normalizeAddress(change.issuer),
          value,
        }
      })
      .filter(Boolean)
...
const getAnyIouBalanceDelta = (meta, account) => {
  const deltas = getPositiveIouDeltas(meta, account)
  if (deltas.length !== 1) {
    if (deltas.length > 1) {
      logWarn('Multiple IOU deltas found', {
        account: normalizeAddress(account),
        deltas: deltas.map((delta) => ({
          currency: delta.currency,
          issuer: delta.issuer,
          value: delta.value.toString(),
        })),
      })
    }
    return null
...
const getIouBalanceDelta = (meta, account, currency, issuer) => {
  ...
  try {
    const accountKey = normalizeAddress(account)
    const accountChanges = getAccountBalanceChanges(meta, accountKey)
    ...
    if (!entry?.value) {
      logWarn('Escrow delta not found', {
        account: accountKey,
        targetCurrency,
        targetIssuer,
        available: accountChanges.map((change) => ({
          currency: normalizeCurrency(change.currency),
          issuer: normalizeAddress(change.issuer),
          value: change.value,
        })),
...
const processStakeTxEntry = async (entry, metaData, lpTokenCurrency, lpTokenIssuer, escrowAddress) => {
  ...
  const txHash = txFields.hash || entry?.hash || entry?.tx?.hash || entry?.tx_json?.hash
  ...
  const fallbackDelta =
    !delta && !amountMatches
      ? getPositiveIouDeltas(metaData, escrowAddress).find(
          (entry) =>
            entry.currency === normalizeCurrency(lpTokenCurrency) &&
            entry.issuer === normalizeAddress(lpTokenIssuer),
        )
      : null
  const stakeValue =
    delta ||
    (amountMatches && amount?.value ? new BigNumber(amount.value) : null) ||
    (fallbackDelta ? fallbackDelta.value : null)
  if (!stakeValue || !stakeValue.isFinite() || stakeValue.lte(0)) {
    return false
  }
  if (fallbackDelta) {
    logWarn('Stake asset fallback (indexer)', {
      txHash,
      currency: fallbackDelta.currency,
      issuer: fallbackDelta.issuer,
      value: fallbackDelta.value.toString(),
    })
  }
...
const indexEscrowStakes = async () => {
  ...
  try {
    ...
    for (const entry of transactions) {
      ...
      const metaData = entry?.meta || entry?.metaData || entry?.tx?.meta
...
const verifyAction = async ({ account, action, farmId, txHash, destinations, expectAmount }) => {
  ...
  if (expectAmount?.type === 'iou') {
    ...
    if (delta) {
      ...
      logWarn('LP delta not found in realtime tx, deferring to indexer', {
        txHash,
        destination,
        expectedCurrency,
        expectedIssuer,
...
const applyUnstakeDb = async ({ account, amount, txHash, lpTokenCurrency, lpTokenIssuer }) => {
  ...
  return withTransaction(async (client) => {
    ...
    const applied = await recordActionDb(client, account, 'unstake', actual, txHash)
    if (!applied) {
      return {
        stakedBalance: formatValue(userState.balance),
        totalStaked: formatValue(farmState.totalStaked),
...
const mountFarmsApi = (app) => {
  ...
  app.get(`${API_PREFIX}/farms/lp-trustline`, async (req, res) => {
    try {
      const account = normalizeAddress(req.query.account)
      if (!account) {
      ...
      res.json({
        tx: buildLpTrustlineTx(account, meta.lpTokenCurrency, meta.lpTokenIssuer),
      })
    } catch (error) {
      res.status(500).json({ error: error.message || 'Failed to build trustline tx' })
    }
  })
  ...
  app.post(`${API_PREFIX}/farms/stake`, async (req, res) => {
    try {
      ...
      const fallbackDelta = !stakeDelta
        ? getPositiveIouDeltas(verified?.meta, escrowAddress).find(
            (entry) =>
              entry.currency === normalizeCurrency(meta.lpTokenCurrency) &&
              entry.issuer === normalizeAddress(meta.lpTokenIssuer),
          )
        : null
      const stakeAmount =
        verified?.amount?.value || stakeDelta?.toString() || fallbackDelta?.value?.toString()
      if (fallbackDelta) {
        logWarn('Stake asset fallback (realtime)', {
          txHash,
          currency: fallbackDelta.currency,
          issuer: fallbackDelta.issuer,
          value: fallbackDelta.value.toString(),
      ...
      if (!stakeAmount || new BigNumber(stakeAmount).lte(0)) {
        logWarn('Stake pending, awaiting indexer', { txHash, account })
        res.json({ pending: true, message: 'Stake detected, indexing in progress', txHash })
        return
      }
  if (isDbEnabled()) {
  ...
  app.post(`${API_PREFIX}/farms/unstake`, async (req, res) => {
    try {
      ...
      const hasLine = await hasTrustline(account, meta.lpTokenIssuer, meta.lpTokenCurrency)
      if (!hasLine) {
        res.status(400).json({ error: 'LP trustline required', action: 'create_lp_trustline' })
        return
      }
  ...
  app.post(`${API_PREFIX}/farms/indexer/tx`, async (req, res) => {
    try {
      ...
      const metaData = tx?.meta || tx?.metaData || tx?.tx_json?.meta
      const applied = await processStakeTxEntry(
        tx,
        metaData,
        meta.lpTokenCurrency,
